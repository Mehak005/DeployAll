// expressjs-and-rest-Mehak005/app.js
...
app.post('/api/students', (req, res) => {
  ...
  db.run(
    "INSERT INTO students (name, favouriteColor, favouriteFood, likes) VALUES (?, ?, ?, 0)",
    [name, favouriteColor, favouriteFood],
    function (err) {
      if (err) return res.status(500).json({ error: err.message });
      res.status(201).json({
        id: this.lastID,
...
app.put('/api/students/:id', (req, res) => {
  ...
  db.run(
    "UPDATE students SET name = ?, favouriteColor = ?, favouriteFood = ?, likes = ? WHERE id = ?",
